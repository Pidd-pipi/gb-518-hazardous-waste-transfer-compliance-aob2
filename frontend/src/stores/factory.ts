...
export class EntityStore {
  ...
  notifyError(message: string) { this.patch({ error: message }); }
  ...
  async transition(path: string, item: DomainRecord, status: string, reason = '前端工作台人工确认') { this.patch({ loading: true }); try { await request<DomainRecord>(`/${path}/${item.id}/transition`, { method: 'POST', body: JSON.stringify({ status, expectedVersion: item.version, reason }) }); await this.load(path); } catch (error) { this.patch({ loading: false, error: error instanceof Error ? error.message : String(error) }); throw error; } }
