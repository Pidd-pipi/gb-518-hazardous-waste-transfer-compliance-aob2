...
export type CheckState = 'pending' | 'pass' | 'fail' | 'rectifying' | 'recheck_pending' | 'escalated';
export const ALL_CHECK_STATE: readonly CheckState[] = ['pending', 'pass', 'fail', 'rectifying', 'recheck_pending', 'escalated'];
...
export const ENTITY_CONFIGS: readonly EntityConfig[] = [
  ...
  { key: 'complianceCheck', path: 'checks', label: '合规核验', statuses: ['pending', 'pass', 'fail', 'rectifying', 'recheck_pending', 'escalated'] as const, transitionRole: 'reviewer' }
];
...
export const TRANSITIONS: Readonly<Record<string, Readonly<Record<string, readonly string[]>>>> = {
  ...
  // fail 后的整改/复检由核验页整改面板的专用操作驱动，不在通用迁移按钮中暴露。
  complianceCheck: { pending: ['pass', 'fail'], pass: [], fail: ['escalated'], rectifying: ['escalated'], recheck_pending: ['escalated'], escalated: [] }
};
