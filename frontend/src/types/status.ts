...
export const ENTITY_CONFIGS: readonly EntityConfig[] = [
  ...
  { key: 'complianceCheck', path: 'checks', label: '合规核验', statuses: ['pending', 'pass', 'fail', 'pending_reinspection', 'escalated'] as const, transitionRole: 'reviewer' }
];
...
export const TRANSITIONS: Readonly<Record<string, Readonly<Record<string, readonly string[]>>>> = {
  ...
  complianceCheck: { pending: ['pass', 'fail'], pass: [], fail: ['escalated'], pending_reinspection: [], escalated: [] }
};
